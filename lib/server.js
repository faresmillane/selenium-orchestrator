    ...
    try {
        ...
        for (let i = 0; i < config["nodes"].length; i++) {
            if(config["nodes"][i].active === true){
                if(config["nodes"][i].driverName === 'appium' && clear.getPlatform() === "windows"){
                    await exec(`yarn appium:start &`);
                } else if (config["nodes"][i].driverName === 'appium' && clear.getPlatform() != "windows"){
